  ...
  try {
    ...
    for (let i = 0; i < cart.length; i++) {
      ...
      const row = sheet.addRow([
        ...
        { formula: `IF(ISBLANK(H${tableHeaderRow.number + 1 + i}), "", E${tableHeaderRow.number + 1 + i} * H${tableHeaderRow.number + 1 + i})` }
      ]);
    ...
    if (resend) {
      ...
      console.log("Mock Resend Send (no RESEND_API_KEY):", options.subject);
