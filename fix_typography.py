...
new_section = """<section class="method-list" style="display: grid; gap: 24px; padding: 40px 5vw; max-width: 1200px; margin: 0 auto;">
  <style>
    ...
    .flip-card { aspect-ratio: auto; min-height: 240px; cursor: pointer; perspective: 1000px; }
    ...
    .flip-card:nth-child(odd) .flip-card-front { border-left: 8px solid var(--yellow); }
    .flip-card:nth-child(odd) .flip-card-front .card-inner-box { background: var(--green); color: #fff; }
    
    /* LEFT COLUMN BACK */
    .flip-card:nth-child(odd) .flip-card-back { border-left: 8px solid var(--yellow); }
    .flip-card:nth-child(odd) .flip-card-back .card-inner-box { background: #fff; border: 2px solid var(--green); color: var(--ink); align-items: center; text-align: center; }
    .flip-card:nth-child(odd) .flip-card-back .method-title { color: var(--green); }
    
    /* RIGHT COLUMN (EVEN CARDS) -> Inner Box YELLOW, Border GREEN */
    .flip-card:nth-child(even) .flip-card-front { border-left: 8px solid var(--green); }
    .flip-card:nth-child(even) .flip-card-front .card-inner-box { background: var(--yellow); color: var(--ink); }
    
    /* RIGHT COLUMN BACK */
    .flip-card:nth-child(even) .flip-card-back { border-left: 8px solid var(--green); }
    .flip-card:nth-child(even) .flip-card-back .card-inner-box { background: #fff; border: 2px solid var(--yellow); color: var(--ink); align-items: center; text-align: center; }
