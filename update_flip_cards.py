...
new_section = """<section class="method-list" style="display: grid; gap: 24px; padding: 40px 5vw; max-width: 1200px; margin: 0 auto;">
  <style>
    ...
    .flip-card { min-height: 220px; cursor: pointer; perspective: 1000px; }
    ...
    .flip-card-front, .flip-card-back {
      ...
      padding: 32px 40px; display: flex; flex-direction: column; justify-content: center;
      color: #fff;
    }
    .flip-card-back { transform: rotateY(180deg); }
    
    /* LEFT COLUMN (ODD CARDS: 1, 3, 5, 7) -> GREEN */
    .flip-card:nth-child(odd) .flip-card-front,
    .flip-card:nth-child(odd) .flip-card-back { 
      background: var(--green); 
    }
    
    /* RIGHT COLUMN (EVEN CARDS: 2, 4, 6, 8) -> YELLOW */
    .flip-card:nth-child(even) .flip-card-front,
    .flip-card:nth-child(even) .flip-card-back { 
      background: var(--yellow); 
    }
    
    .method-num { font-size: 15px; font-weight: 800; margin-bottom: 8px; display: block; opacity: 0.9; }
    .method-title { font-size: 24px; font-weight: 900; margin-bottom: 12px; text-transform: uppercase; line-height: 1.1; font-family: 'Acumin Variable Concept', 'Acumin Pro Condensed', Montserrat, sans-serif; letter-spacing: 1px; }
    .method-desc { font-size: 14px; opacity: 0.95; line-height: 1.5; margin: 0; font-weight: 500; }
  </style>
"""
  ...
  <article class="flip-card">
    <div class="flip-card-inner">
      ...
      <div class="flip-card-back">
        <span class="method-num" style="opacity:0.7;">{num} - Expanded</span>
        <h2 class="method-title">{title}</h2>
