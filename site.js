...
window.handleImgError = function(img, fallbackSrc = '') {
  ...
  // Never hide on empty src, data urls, or matching page URL
  if (!rawSrc || rawSrc.startsWith('data:') || rawSrc === window.location.href) {
    return;
  }

  // 1. If image has a high-res base64 backupSrc, immediately fallback to it
...
function applySiteSettings() {
  ...
  if (sc.contactHQTitle) {
    const el = document.getElementById('cmsContactHQTitle');
    if (el) el.textContent = sc.contactHQTitle;
  }
  if (sc.contactGlobalTitle) {
    const el = document.getElementById('cmsContactGlobalTitle');
    if (el) el.textContent = sc.contactGlobalTitle;
  }
  if (sc.contactEmailTitle) {
    const el = document.getElementById('cmsContactEmailTitle');
  ...
  // Social Links Engine - Dynamic Square Social Icons
  const SOCIAL_PRESET_SVGS = {
  ...
  document.querySelectorAll('#cmsSocialLinks, .footer-social-links').forEach(el => socialContainers.add(el));
  document.querySelectorAll('#cmsSocialLinkedIn, #cmsSocialInstagram, #cmsSocialFacebook').forEach(el => {
    if (el.parentElement) socialContainers.add(el.parentElement);
  });

  const rawSocialIcons = (sc.socialIcons && Array.isArray(sc.socialIcons)) ? sc.socialIcons : [
    { id: 'linkedin', name: 'LinkedIn', iconKey: 'linkedin', url: sc.socialLinkedIn || '', active: !!sc.socialLinkedIn },
    { id: 'facebook', name: 'Facebook', iconKey: 'facebook', url: sc.socialFacebook || '', active: !!sc.socialFacebook },
    { id: 'instagram', name: 'Instagram', iconKey: 'instagram', url: sc.socialInstagram || '', active: !!sc.socialInstagram }
  ];

  const activeSquareIcons = rawSocialIcons.filter(item => item && item.url && item.url.trim() && item.url !== '#' && item.active !== false);

  socialContainers.forEach(container => {
    if (activeSquareIcons.length > 0) {
      container.innerHTML = activeSquareIcons.map(icon => {
        let iconSvg = '';
        if (icon.iconImg) {
          iconSvg = `<img src="${icon.iconImg}" alt="${icon.name || 'Social'}" style="width: 18px; height: 18px; object-fit: contain;" />`;
        } else {
          const key = (icon.iconKey || icon.name || '').toLowerCase().trim();
          iconSvg = SOCIAL_PRESET_SVGS[key] || icon.iconSvg || SOCIAL_PRESET_SVGS.globe;
        }
        return `<a href="${icon.url}" target="_blank" rel="noopener noreferrer" title="${icon.name || 'Social'}" class="square-social-btn" style="display: inline-flex; width: 34px; height: 34px; background: #222222; border: 1px solid #3a3a3a; border-radius: 6px; align-items: center; justify-content: center; color: #ffffff; text-decoration: none; transition: all 0.2s ease; margin-right: 8px; margin-bottom: 6px; box-sizing: border-box;" onmouseover="this.style.background='#2f873d';this.style.borderColor='#2f873d';this.style.transform='translateY(-2px)';" onmouseout="this.style.background='#222222';this.style.borderColor='#3a3a3a';this.style.transform='none';">${iconSvg}</a>`;
      }).join('');
      ...
      // Fallback: hide legacy elements if none active
      container.querySelectorAll('#cmsSocialInstagram, #cmsSocialFacebook, #cmsSocialLinkedIn').forEach(el => el.style.display = 'none');
    }
  });
...
function saveCart() {
  try {
    ...
    console.warn("Storage quota warning in saveCart, saving safe cart items...", err);
    try {
...
function triggerMailtoFallback(customerInfo, cart) {
  ...
  if (!cart || cart.length === 0) {
    ...
    cart.forEach(item => {
      const hasLogo = !!(item.artworkSrc || (item.logoData && (item.logoData.imageSrc || item.logoData.data)));
      bodyText += `- ${item.name || 'Product'} (${item.sku || 'N/A'})\n  Size: ${item.size || "N/A"} | Color: ${item.color || "Standard"} | Qty: ${item.quantity || 1}\n  Branding: ${item.branding || "None"}${hasLogo ? '\n  Logo File: [Customer Uploaded Custom Logo Attached]' : ''}\n\n`;
    });
  }
...
$("#quoteForm")?.addEventListener("submit", async (event) => {
  ...
  if (dateInput) {
    ...
    dateInput.style.borderColor = 'var(--line)';
  }
  ...
  try {
    ...
    const sanitizedCart = await Promise.all((cart || []).map(async (item, cIdx) => {
      ...
      // 4. CRITICAL: Explicitly push customer's uploaded logo to attachments array so it is guaranteed to send with the order
      const logoToAttach = cleanItem.artworkSrc || (cleanItem.logoData && cleanItem.logoData.imageSrc);
      if (logoToAttach && typeof logoToAttach === 'string') {
        const logoCleanB64 = logoToAttach.includes('base64,') ? logoToAttach.split('base64,')[1].trim() : logoToAttach.trim();
        if (logoCleanB64) {
          const itemSku = (cleanItem.sku || `Item_${cIdx + 1}`).replace(/[^a-zA-Z0-9_-]/g, '_');
          attachments.push({
            filename: `Item_${cIdx + 1}_${itemSku}_Custom_Logo.png`,
            content: logoCleanB64,
            isArtwork: true
          });
        }
      }
...
function initDeliveryDatePicker() {
  ...
  const dPicker = document.getElementById('deliveryDatePicker');
  const dCalBtn = document.getElementById('deliveryDateCalBtn');
  const dError = document.getElementById('deliveryDateError');
  ...
  if (dCalBtn && dPicker) {
    dCalBtn.addEventListener('click', (e) => {
      e.preventDefault();
      try {
        if (typeof dPicker.showPicker === 'function') {
          dPicker.showPicker();
        } else {
          dPicker.focus();
          dPicker.click();
